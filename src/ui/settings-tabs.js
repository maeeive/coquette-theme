...
export function createTabbedSettingsUI(container, settings) {
    ...
    const tabs = [
        ...
        { id: 'mobile-devices', label: t`Mobile Devices ☁️` },
    ];
...
function populateTabContent(tabs, tabContents, settings, firstSectionAlwaysExpanded = false) {
    ...
    tabs.forEach(tab => {
        ...
        categories.forEach(category => {
            ...
            sectionTitle.style.color = '#ff99a8'; /* Rosa bebé nos títulos das secções */
...
function getCategoryDisplayName(category) {
    ...
    const categoryNames = {
        ...
        'background-effects': t`Background Effects ☁️`,
        'theme-extras': t`Theme Extras 𖹭`,
        'raw-css': t`Advanced Custom CSS ♡`,
        'chat-general': t`General Chat Settings ✿`,
        'visual-novel': t`Visual Novel Mode 𜗿`,
        'chat-echo': t`Echo Style Settings ☁️`,
        'chat-whisper': t`Whisper Style Settings 🎀`,
        'chat-ripple': t`Ripple Style Settings 𖹭`,
        'mobile-global-settings': t`Mobile Global Settings ☁️`,
        ...
        'features': t`Advanced Features 𖹭`,
        'general': t`General Settings ♡`,
        'mobileSettings': t`Mobile Device Settings ☁️`,
    };
...
function addTabStyles() {
    ...
    styleElement.textContent = `
        ...
        .moonlit-tab-buttons {
            display: flex;
            border-bottom: 2px dashed #ffb6c1; /* Linha de divisão tracejada */
            margin-bottom: 15px;
        ...
        .moonlit-tab-button {
            padding: 10px 15px;
            background: rgba(255, 240, 245, 0.4); /* Fundo pastel fofo */
            border: 1px dashed transparent;
            border-bottom: none;
            border-radius: 15px 15px 0 0; /* Abas arredondadas em cima */
            cursor: pointer;
            color: #ff99a8;
            font-weight: bold;
            opacity: 0.8;
        ...
        .moonlit-tab-button:hover {
            ...
            transform: translateY(-2px); /* Efeito pulinho */
        }

        .moonlit-tab-button.active {
            opacity: 1;
            background: rgba(255, 240, 245, 1);
            border: 2px dashed #ffb6c1; /* Borda Coquette ao selecionar */
            border-bottom: 2px solid rgba(255, 240, 245, 1); /* Une a aba com o fundo */
            color: #ff6b81;
...
function addCollapsibleSectionStyles() {
    ...
    styleElement.textContent = `
    .moonlit-section {
        border: 2px dashed #ffb6c1; /* Borda tracejada principal */
    ...
    .moonlit-section-header {
        background-color: rgba(255, 182, 193, 0.2); /* Fundo do título */
        padding: 8px 15px;
    ...
    .checkbox-container input[type="checkbox"] {
        width: 20px; /* Checkbox um bocadinho maior */
        height: 20px;
        cursor: pointer;
        margin-left: auto;
        margin-right: unset;
        accent-color: #ffb6c1; /* Cor rosa quando está selecionado */
    }
