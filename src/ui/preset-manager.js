...
export function createPresetManagerUI(container, settingsOverride) {
    ...
    presetManagerContainer.style.marginBottom = '15px';
    presetManagerContainer.style.padding = '15px'; /* Espaço interno fofo */
    presetManagerContainer.style.background = 'rgba(255, 240, 245, 0.6)'; /* Vidro rosa */
    presetManagerContainer.style.borderRadius = '20px'; /* Nuvem */
    presetManagerContainer.style.border = '2px dashed #ffb6c1'; /* Tracejado coquette */

    const presetTitle = document.createElement('h4');
    presetTitle.textContent = t`Moonlit Echoes Presets 🎀`;
    presetTitle.style.marginBottom = '10px';
    presetTitle.style.marginTop = '0';
    presetTitle.style.color = '#ff99a8';
    presetTitle.style.borderBottom = '2px dashed rgba(255, 182, 193, 0.5)';
    presetTitle.style.paddingBottom = '8px';
    ...
    presetSelector.style.borderRadius = '15px';
    presetSelector.style.border = '1px solid #ffb6c1';
    presetSelector.style.padding = '5px';
    presetSelector.style.color = '#ff99a8';
    ...
    const createMenuButton = (id, title, iconClass) => {
        const btn = document.createElement('button');
        btn.id = id;
        btn.classList.add('menu_button');
        btn.title = title;
        btn.innerHTML = `<i class="${iconClass}"></i>`;
        btn.style.color = '#ff99a8';
        btn.style.borderRadius = '15px';
        btn.style.transition = 'all 0.3s ease';
        btn.onmouseover = () => { btn.style.transform = 'scale(1.1)'; btn.style.color = '#ff6b81'; };
        btn.onmouseout = () => { btn.style.transform = 'scale(1)'; btn.style.color = '#ff99a8'; };
        return btn;
    };

    const importButton = createMenuButton('moonlit-preset-import', t`Import Preset`, 'fa-solid fa-file-import');
    importButton.addEventListener('click', importPreset);
    buttonsRow.appendChild(importButton);

    const exportButton = createMenuButton('moonlit-preset-export', t`Export Preset`, 'fa-solid fa-file-export');
    exportButton.addEventListener('click', exportActivePreset);
    buttonsRow.appendChild(exportButton);

    const saveButton = createMenuButton('moonlit-preset-save', t`Update Current Preset`, 'fa-solid fa-save');
    saveButton.addEventListener('click', updateCurrentPreset);
    buttonsRow.appendChild(saveButton);

    const newButton = createMenuButton('moonlit-preset-new', t`Save as New Preset`, 'fa-solid fa-file-circle-plus');
    newButton.addEventListener('click', saveAsNewPreset);
    buttonsRow.appendChild(newButton);

    const deleteButton = createMenuButton('moonlit-preset-delete', t`Delete Preset`, 'fa-solid fa-trash-can');
...
export function saveAsNewPreset() {
    import('../../../../../popup.js').then(({ POPUP_TYPE, callGenericPopup }) => {
        callGenericPopup(
            `<h3 style="color: #ff99a8;">✿ <span data-i18n="Save New Moonlit Echoes Theme Preset">Save New Moonlit Echoes Theme Preset</span> ♡</h3>
            ...
            if (settings.presets[presetName]) {
                import('../../../../../popup.js').then(({ POPUP_TYPE, callGenericPopup }) => {
                    callGenericPopup(
                        `<h3 style="color: #ff99a8;">𜗿 <span data-i18n="Confirm Overwrite">Confirm Overwrite</span> ☁️</h3>
...
export function deleteCurrentPreset() {
    ...
    import('../../../../../popup.js').then(({ POPUP_TYPE, callGenericPopup }) => {
        callGenericPopup(
            `<h3 style="color: #ff99a8;">♡ ${managerConfig.t`Delete Theme Preset`} 𖹭</h3><p>${managerConfig.t`Are you sure you want to delete the preset "${presetName}"?`}</p>`,
            POPUP_TYPE.CONFIRM
        ).then((confirmed) => {
            if (!confirmed) return;
